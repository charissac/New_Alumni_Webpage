...
<div style = "background: rgba(255, 255, 255, 0.9);">
  <h2 style = "color: #7c795d; font-family: 'Trocchi', serif; font-size: 30px; font-weight: normal; line-height: 48px; margin: 0;text-align: center;"> Cancel Friend Request</h2>
  <div align="center">
  
   <form action="process_cancelFriendRequest.php" method="post" enctype="multipart/form-data">
...
$friendRequest = mysqli_query($conn,"SELECT * FROM friend WHERE status = '1' AND fromUserID = ".$_SESSION['user']);

$friendRow=mysqli_fetch_array($friendRequest);

//$result = mysql_query("SELECT userID,fullName,programme,major,graduateYear,evidence FROM user WHERE status=1");


//$result = mysql_query("SELECT userID,fullName,programme,major,graduateYear FROM user WHERE userID=" .$friendRow['toUserID']);
$number = 1;
...
echo "  <tr>
		<td><input type='checkbox' name='userID[".$number."]' value='". $row['0'] ."' /></td>
		<td> <a href='cancelProfileFriendRequest.php?uid=".$row['0']."'> ".$row['1']." </a> </td>
		<td>".$row['2']."</td>
		<td>  ".$row['3']."  </td> 
		<td>  ".$row['4']."  </td> 
...
echo "</br>";
echo "</br>";
echo " <input class='btn btn-primary' name='action'  type='submit' value='Cancel' onclick= 'return confirm('Are you sure want to cancel this?')' /> <font face='verdana'> ";
...
<!--<input class="btn btn-primary" type="submit"  value="Cancel" name="action" class="cancel" onclick="return confirm('Are you sure want to cancel this?')" /> <font face="verdana">
-->
