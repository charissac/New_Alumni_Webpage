...
<!--	<p> 	<label><font color="purple"><b>Email:  </b></font> </label> 
	...
	<p> 	<label><font color="purple"><b>Programme:  </b></font> </label>
		...
		<p> <label><font color="purple"><b>Message:  </b></font> </label> <textarea rows="3" cols="70" name="content" required />  </textarea> </p>
	   ...
	   <label style="visibility: hidden">  Message:  </label>
		...
		$wall = mysqli_query($conn,"SELECT toFriendName, fromFriendName, comment, dateTime FROM postwall WHERE toUserID = '". $userRow['userID'] ."' AND fromUserID= '".$_SESSION['user']."' 
		...
		if (mysqli_num_rows($wall ) > 0)	{		
			while ($row = mysqli_fetch_array($wall, MYSQL_NUM)) {
				echo "<label style='visibility: hidden'>  Message:  </label>";
				echo "<p> <table style = 'width:54%'>";
				echo "  <tr><td>".$row['1']." >" .$row['0']. "</td> <tr/>
